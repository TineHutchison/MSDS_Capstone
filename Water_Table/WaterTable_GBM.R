...
#as.numeric(water_table$public_meeting)
#table(water_table$permit, water_table$source)
...
#table(water_table$"public_meeting")
...
gbm_train <- water_table_train[,fieldList]
gbm_train <- add_dummies(gbm_train, water_table_train)


gbm_train$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table_train$date_recorded )
randomForest_data_full$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table$date_recorded )
randomForest_data_test$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table_test$date_recorded )


gbm_model <- gbm(water_table_train_y$status_group ~ .
            , data=gbm_train, n.trees=1000, cv.folds = 0)
### cv.folds to 0 otherwise it fails on factor levels being different based on the link:  
### https://stackoverflow.com/questions/25514484/error-in-r-gbm-function-when-cv-folds-0


1-sum(gbm_model$oobag.improve)

plot(gbm_model$oobag.improve)
...
gbm_holdout <- water_table_holdout[,fieldList]
gbm_holdout <- add_dummies(gbm_holdout, water_table_holdout)

gbm_holdout$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table_holdout$date_recorded) 

gbm_predict <- data.frame(predict.gbm(gbm_model, gbm_holdout, type="response", n.trees = 1000))
colnames(gbm_predict) <- c("functional","functional needs repair", "non functional")


#plot(gbm_model$oobag.improve)
gbm_values <- apply(gbm_predict,1, getMax)
gbm_values <- factor(gbm_values)
gbm_values <- factor(gbm_values, levels=c("functional","functional needs repair", "non functional"))


confusionMatrix(water_table_holdout_y$status_group, gbm_values)



#### retrain with full model

gbm_full <- water_table[,fieldList]
gbm_full <- add_dummies(gbm_full, water_table)
gbm_full$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table$date_recorded )


gbm_test <- water_table_test[,fieldList]
gbm_test <- add_dummies(gbm_test, water_table_test)
gbm_test$days <- as.integer(as.Date("14/01/01", format="%y/%m/%d") - water_table_test$date_recorded )



gbm_model_full <- gbm(water_table_y$status_group ~ .
                 , data=gbm_full, n.trees=2500, cv.folds = 0)


1-sum(gbm_model_full$oobag.improve)


gbm_predict <- data.frame(predict.gbm(gbm_model_full, gbm_test, type="response", n.trees = 2500))
colnames(gbm_predict) <- c("functional","functional needs repair", "non functional")
...
gbm_predict_values <- data.frame(gbm_values)
head(gbm_predict_values)
writeResultsFileForSubmission("gbm, ntrees=2500",gbm_predict_values)
