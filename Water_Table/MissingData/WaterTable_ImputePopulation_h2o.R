...
sum(!water_table$has_population)
...
logpopTree <- tree(data_train$logpop ~ .,data=data_train)
plot(data_holdout$logpop, predict(logpopTree, data_holdout))
mean(abs(data_holdout$logpop- predict(logpopTree, data_holdout)))
...
#randomForest_logpop <- h2o.randomForest(features,"logpop", trainH2o, ntrees=100, mtries=10, max_depth=22, nbins=26, seed=42)
randomForest_logpop <- h2o.randomForest(features,"logpop", trainH2o, ntrees=100, mtries=10, max_depth=22, nbins=26, seed=42)

#plot(randomForest_logpop$mse)
predictionH2o <- predict(randomForest_logpop, holdoutH2o)
#colnames(as.data.frame(predictionH2o$predict))
plot(as.data.frame(predictionH2o$predict)$predict, data_holdout$logpop)
mean(abs(as.data.frame(predictionH2o$predict)$predict- data_holdout$logpop))
#### logpop: 0.6669213
#### updated: 
...
randomForest_AllData <- h2o.grid("randomForest", nfolds = 5, grid_id = "mygrid",  x=features,hyper_params = hyper_params,y="logpop", training_frame=trainH2o, seed=42, ntrees=100)
...
randomForest_logpop <- h2o.randomForest(features,"logpop", trainH2o, ntrees=100, mtries=9, max_depth=21, nbins=21,  seed=42)

show(randomForest_logpop)


missingH2o<-as.h2o(data_missing)
missingH2o<-as.h2o(missingH2o)
predictions  <- predict(randomForest_logpop, missingH2o)
water_table[water_table$has_population==0,]$logpop_imp <- as.data.frame(predictions)$predict


missingH2o<-as.h2o(data_missing_test)
missingH2o<-as.h2o(missingH2o)
predictions  <- predict(randomForest_logpop, missingH2o)
water_table_test[water_table_test$has_population==0,]$logpop_imp <- as.data.frame(predictions)$predict
